...
/****dissipative flux****/
		...
		fdiss1 = (beta*L_max*phi+Movers(rhour, rhoul, rhor, rhol, L_max, L_min))*(rhor-rhol);		
		fdiss2 = (beta*L_max*phi+Movers(pr + rhour*ur, pl + rhoul*ul, rhour, rhoul, L_max, L_min))*(rhour-rhoul);
		fdiss3 = (beta*L_max*phi+Movers(rhour*hr, rhoul*hl, rhor*Er, rhol*El, L_max, L_min))*(rhor*Er-rhol*El);
...
T Movers(T Fr, T Fl, T Ur,T Ul,T L_max, T L_min){
	...
	else if (fabs(Ur-Ul)<epsilon and fabs(Fr-Fl)>epsilon ) return L_min;
	else S=L_min;
